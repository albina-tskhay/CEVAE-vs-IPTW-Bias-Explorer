...
class Model(PyroModule):
    """
    Generative model for a causal model with multiple latent confounders `z`.
    """

    def __init__(self, config):
        self.latent_dim = config["latent_dim"]  # Allow multiple latent confounders
        super().__init__()
        self.x_nn = DiagNormalNet(
            [self.latent_dim] + [config["hidden_dim"]] * config["num_layers"] + [config["feature_dim"]]
        )
        OutcomeNet = DistributionNet.get_class(config["outcome_dist"])
        self.y0_nn = OutcomeNet([self.latent_dim] + [config["hidden_dim"]] * config["num_layers"])
        self.y1_nn = OutcomeNet([self.latent_dim] + [config["hidden_dim"]] * config["num_layers"])
        self.t_nn = BernoulliNet([self.latent_dim])
    ...
    def y_mean(self, x, t):
        """Estimate the mean outcome for given x and t, incorporating latent confounders."""
        with pyro.plate("data", x.size(0)):
            z = pyro.sample("z", self.z_dist())  # Sample multiple latent confounders
            x = pyro.sample("x", self.x_dist(z), obs=x)
            t = pyro.sample("t", self.t_dist(z), obs=t)
        return self.y_dist(t, z).mean

    def z_dist(self):
        # Allow multiple latent confounders by using self.latent_dim
...
class CEVAE(nn.Module):
    ...
    def ite(self, x, num_samples=None, batch_size=None):
        ...
        for x in dataloader:
            ...
            with pyro.plate("num_particles", num_samples, dim=-2):
                ...
                # Explicitly provide `t=0` and `t=1`
                with poutine.do(data=dict(t=torch.zeros(x.shape[0]))):
                    y0 = poutine.replay(self.model.y_mean, tr.trace)(x, torch.zeros(x.shape[0]))

                with poutine.do(data=dict(t=torch.ones(x.shape[0]))):
                    y1 = poutine.replay(self.model.y_mean, tr.trace)(x, torch.ones(x.shape[0]))
    ...
    def to_script_module(self):
        ...
        """
        self.train(False)
        fake_x = torch.randn(10, self.feature_dim)
